...
window.fetchItems = async () => {
    ...
    try {
        const res = await fetch('/api/v1/company-positions?page=1&limit=99999');
        const data = await res.json();
        if (data.success) {
            state.items = data.positions.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
            applyFilters();
            updateStats();
        }
    } catch (e) {
        showToast('无法连接服务器或加载岗位列表', 'error');
...
function renderTable() {
    ...
    if (!list.length) {
        tbody.innerHTML = `<tr><td colspan="4" class="text-center" style="padding:40px;color:#9ca3af;">没有找到匹配的岗位</td></tr>`;
        return;
    }
...
window.openCreateItemModal = () => {
    ...
    $('#form-modal-title').innerText = '新增岗位';
...
window.openEditItemModal = (id) => {
    const item = state.items.find(i => i.id == id); // Use == for ID comparison (string vs number)
    if (!item) {
        showToast('岗位未找到', 'error');
        return;
    }

    state.editItemId = id;
    $('#form-modal-title').innerText = '编辑岗位';
...
window.handleFormSubmit = async () => {
    ...
    const url = state.editItemId ? `/api/v1/company-positions/${state.editItemId}` : '/api/v1/company-positions';
    ...
    try {
        ...
        if (result.success) {
            showToast(state.editItemId ? '岗位更新成功' : '岗位创建成功');
            closeModal('form-modal');
            fetchItems();
        } else {
...
window.deleteItem = async (id) => {
    const confirmed = await showConfirmModal('确认删除?', '此岗位将被<b>永久删除</b>，且无法恢复。您确定要继续吗？');
    if (!confirmed) return;
    try {
        const res = await fetch(`/api/v1/company-positions/${id}`, { method: 'DELETE' });
        if (res.ok) {
            showToast('岗位已成功删除');
            fetchItems();
        } else {
